...
# 🔐 Sadece Yönetici / Kurucunun bildiği Koç Kayıt Katılım Kodu
...
def halka_grafik_html(baslik, veri_listesi):
    """HTML/CSS Conic Gradient Halka Grafik Üreticisi"""
    toplam = sum([v for _, v, _ in veri_listesi])
    if toplam == 0:
        return "<p style='color:#666; text-align:center; padding:15px;'>Henüz görüntülenecek soru verisi kaydedilmedi.</p>"
    ...
    for etiket, deger, renk in veri_listesi:
        ...
        item_str = (
            f'<div style="display:flex; align-items:center; margin-bottom:6px; font-size:13px; font-family:sans-serif;">'
            f'<span style="width:12px; height:12px; background-color:{renk}; border-radius:50%; display:inline-block; margin-right:8px; flex-shrink:0;"></span>'
            f'<span style="color:#334155; font-weight:600;">{etiket}:</span>&nbsp;'
            f'<span style="color:#0f172a; font-weight:bold;">{deger} Adet</span>&nbsp;'
            f'<span style="color:#64748b; font-size:11px;">(%{yuzde:.1f})</span>'
            f'</div>'
        )
    ...
    raw_html = f'''<div style="background-color:#ffffff; padding:18px; border-radius:12px; border:1px solid #e2e8f0; box-shadow:0 4px 6px -1px rgba(0,0,0,0.05); margin-bottom:15px;">
<h4 style="margin-top:0; margin-bottom:15px; color:#1e293b; font-family:sans-serif; text-align:center; font-size:16px;">{baslik}</h4>
<div style="display:flex; flex-wrap:wrap; align-items:center; justify-content:center; gap:20px;">
<div style="width:140px; height:140px; border-radius:50%; background:conic-gradient({gradient_str}); position:relative; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
<div style="width:80px; height:80px; background:white; border-radius:50%; display:flex; flex-direction:column; align-items:center; justify-content:center; font-family:sans-serif; box-shadow:inset 0 2px 4px rgba(0,0,0,0.06);">
<span style="font-size:18px; font-weight:bold; color:#0f172a;">{toplam}</span>
<span style="font-size:9px; color:#64748b; font-weight:bold; letter-spacing:0.5px;">TOPLAM</span>
</div>
</div>
...
# Veritabanı Otomatik Onarım Kontrolleri
cursor.execute("PRAGMA table_info(ogrenciler)")
o_columns = [row[1] for row in cursor.fetchall()]
if "koc_adi" not in o_columns:
    try:
        cursor.execute("ALTER TABLE ogrenciler ADD COLUMN koc_adi TEXT DEFAULT ''")
        conn.commit()
    except Exception:
        pass
...
cursor.execute("PRAGMA table_info(denemeler)")
d_columns = [row[1] for row in cursor.fetchall()]
if "koc_notu" not in d_columns:
    try:
        cursor.execute("ALTER TABLE denemeler ADD COLUMN koc_notu TEXT DEFAULT ''")
        conn.commit()
    except Exception:
        pass
...
if giris_turu == "👨‍🎓 ÖĞRENCİ GİRİŞİ":
    ...
    with tab_giris:
        ...
        # Sistemdeki Aktif Koçları Listele
        ...
        if st.button("Giriş Yap / Hesabı Oluştur", type="primary", use_container_width=True):
    ...
    if not aktif_ogr:
        ...
        with tab_gunluk:
            ...
            if st.button("🚀 Tüm Ders ve Konu Çalışmalarını Kaydet", type="primary", use_container_width=True):
                ...
                if yeni_kayit:
                    ...
                    st.info("Soru çözümleriniz kaydedildi. Aşağıdaki başarı grafikleriniz anlık olarak güncellendi!")
                else:
                    st.warning("Lütfen kaydetmeden önce en az bir dersten soru sayısı giriniz.")

            st.write("---")
            st.subheader(f"📈 Anlık Soru ve Başarı Grafikleri - {aktif_ogr}")
        ...
        with tab_deneme:
            ...
            st.write("---")
            st.subheader("📋 Geçmiş Denemeleriniz ve Koçunuzun Değerlendirmeleri")
    ...
    if not st.session_state["aktif_koc"]:
        ...
        with koc_tab_giris:
            st.subheader("👨‍🏫 Koç Girişi")
            k_adi_giris = st.text_input("Koç Kullanıcı Adı:", key="k_adi_giris").strip()
            k_sifre_giris = st.text_input("Şifre:", type="password", key="k_sifre_giris")
            
            if st.button("Koç Paneline Giriş Yap", type="primary", use_container_width=True):
        ...
        with koc_tab_kayit:
            st.subheader("📝 Yeni Koç Hesabı Oluştur")
            st.markdown(f"""
            <div class="sifre-bilgi-box">
                🛡️ <strong>Sistem Güvenlik Kuralları:</strong><br/>
                1. Yeni bir koç hesabı açmak için <strong>Sistem Katılım / Yönetici Kodu</strong> zorunludur.<br/>
                2. Koç şifreniz en az 6 karakter olmalı; en az 1 HARF, 1 RAKAM ve 1 NOKTALAMA/ÖZEL KARAKTER (!, ?, ., @, #, vb.) içermelidir.
            </div>
            """, unsafe_allow_html=True)
            
            katilim_kodu_input = st.text_input("🔑 Sistem Katılım / Yönetici Kodu:", type="password", key="katilim_kodu_input")
            yeni_k_adi = st.text_input("Yeni Koç Kullanıcı Adı:", key="yeni_k_adi").strip()
            yeni_k_sifre = st.text_input("Şifre:", type="password", key="yeni_k_sifre")
            yeni_k_sifre_tekrar = st.text_input("Şifre Tekrarı:", type="password", key="yeni_k_sifre_tekrar")
            
            if st.button("Koç Hesabını Kaydet", type="primary", use_container_width=True):
                ...
                elif not yeni_k_adi or not yeni_k_sifre:
        ...
        with koc_tab_sifre:
            st.subheader("🔒 Koç Şifresi Değiştir")
            s_k_adi = st.text_input("Kullanıcı Adınız:", key="s_k_adi").strip()
            eski_sifre = st.text_input("Mevcut Şifreniz:", type="password", key="eski_sifre")
            yeni_sifre = st.text_input("Yeni Şifreniz:", type="password", key="yeni_sifre")
            yeni_sifre_tekrar = st.text_input("Yeni Şifre Tekrarı:", type="password", key="yeni_sifre_tekrar")
        ...
        # YALNIZCA bu Koça atanmış öğrencileri getir
        ...
        if not ogrenciler:
            ...
            st.subheader("📑 Kayıtlı Denemeler ve Koç Değerlendirmesi")
            ...
            if not df_deneme.empty:
                ...
                st.write("---")
                st.markdown("### ✍️ Denemeye Özel Koç Değerlendirmesi & Otomatik AI Analiz Üretici")
                ...
                zayif_str = ", ".join(zayif_liste) if zayif_liste else "Acil müdahale gereken 1-2 puanlık konu yok."
                ...
                with col_pdf:
                    ...
                    html_rapor = f"""
                    <div style="font-family: Arial, sans-serif; padding: 20px; border: 2px solid #3b82f6; border-radius: 10px; background-color: #ffffff; color: #1e293b;">
                        <div style="text-align: center; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px; margin-bottom: 15px;">
                            <h2 style="color: #1e40af; margin: 0;">🎓 YKS ÖĞRENCİ GELİŞİM & DENEME KARNESİ</h2>
                            <p style="color: #64748b; margin: 5px 0 0 0; font-size: 14px;"><strong>Tarih:</strong> {datetime.date.today().strftime('%d.%m.%Y')}</p>
                            <p style="color: #64748b; margin: 2px 0 0 0; font-size: 13px;"><strong>Sorumlu Koç:</strong> {aktif_koc_adi}</p>
                        </div>
                        <p><strong>👨‍🎓 Öğrenci Adı Soyadı:</strong> {secilen_ogr}</p>
                        <p><strong>📑 Deneme Adı / Yayın:</strong> {deneme_row['yayin']} ({deneme_row['tur']})</p>
                        <p><strong>🎯 Toplam Net:</strong> <span style="font-size: 18px; color: #2563eb; font-weight: bold;">{deneme_row['toplam_net']} Net</span></p>
                        <p><strong>🚨 Zayıf Olduğu Konular:</strong> {zayif_str}</p>
                        <div style="background-color: #f1f5f9; padding: 12px; border-radius: 6px; margin-top: 10px;">
                            <strong>👨‍🏫 Koç Analizi & Tavsiyeler:</strong><br/>
                            <p style="white-space: pre-wrap; margin-top: 5px;">{yeni_koc_notu}</p>
                        </div>
                    </div>
                    """
                    b64_html = base64.b64encode(html_rapor.encode('utf-8')).decode('utf-8')
                    href = f'<a href="data:text/html;charset=utf-8;base64,{b64_html}" download="{secilen_ogr}_Deneme_Karnesi.html" style="display: inline-block; padding: 10px 20px; background-color: #10b981; color: white; text-decoration: none; border-radius: 6px; font-weight: bold; text-align: center; width: 100%;">📥 PDF / Raporu İndir (HTML/Yazdır)</a>'
