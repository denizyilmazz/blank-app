                ...
                with st.container():
                    ...
                    <div class="osym-belge-box">
                        <div style="text-align: center; border-bottom: 2px solid #1e293b; padding-bottom: 12px; margin-bottom: 16px;">
                            ...
                            <h4 style="margin:4px 0 0 0; font-weight:700; font-size:15px; color:#334155 !important;">2026 YÜKSEKÖĞRETİMLİLER KURUMLARI SINAVI SONUÇ BELGESİ</h4>
                        </div>
                ...
                if df_ogr_denemeler.empty:
                    ...
                    for _, d_row in df_ogr_denemeler.iterrows():
                        ...
                        <div class="calc-card" style="margin-bottom: 12px;">
                            <div style="font-weight:800; font-size:16px; color:#1e293b;">📌 {d_row['yayin']} ({d_row['tur']}) — Net: {d_row['toplam_net']} <span style="font-size:12px; color:#64748b;">({d_row['tarih']})</span></div>
                        """, unsafe_allow_html=True)
    ...
    with main_tab2:
        ...
        if not st.session_state["aktif_koc"]:
                ...
                with st.form("yeni_koc_tanimla_formu"):
                    ...
                    if st.form_submit_button("Hesabı Oluştur", type="primary", use_container_width=True):
                        if yeni_koc_adi and yeni_koc_sifre and katilim_kodu == SISTEM_YONETICI_KATILIM_KODU:
                            ...
                            st.rerun()
                        else: st.error("❌ Hatalı katılım kodu!")
        else:
            col_k_head1, col_k_head2 = st.columns([0.8, 0.2])
            with col_k_head1:
                st.success(f"🔓 Oturum Açık: **{st.session_state['aktif_koc']}**")
            with col_k_head2:
            ...
            if ogrenci_rows:
                ...
                KOC_MUFREDAT = TYT_KONULAR if "TYT (Sadece" in s_turu else ({**TYT_KONULAR, **AYT_KONULAR} if "YKS" in s_turu else LGS_KONULAR)
                ...
                if df_koc_denemeler.empty:
                    st.info(f"ℹ️ {secilen_ogr} henüz deneme sonucu kaydetmemiştir.")
                else:
                        ...
                        if kd_row['dosya_adi'] != "Dosya Yok" and os.path.exists(kd_row['dosya_adi']):
                            ...
                            with col_f1:
                                with open(kd_row['dosya_adi'], "rb") as f_kd:
                                    st.download_button(f"📥 Karnesini İndir", data=f_kd, file_name=kd_row['dosya_adi'], key=f"dl_koc_karne_{kd_row['id']}")
                            with col_f2:
                ...
                for idx, g_adi in enumerate(GUNLER):
                    with gun_sekmeleri[idx]:
                        ...
                        with col_s1: s_ders = st.selectbox(f"Ders ({g_adi}):", list(KOC_MUFREDAT.keys()), key=f"d_sec_{g_adi}")
                        with col_s2: s_konu = st.selectbox(f"Konu ({g_adi}):", KOC_MUFREDAT[s_ders], key=f"k_sec_{g_adi}")
                        s_saat = st.text_input("Saat Aralığı:", value="09:00 - 10:00", key=f"saat_input_{g_adi}")

                        if st.button(f"✏️ {g_adi} Gününü Güncelle", key=f"btn_add_{g_adi}", type="primary"):
                            icerik = f"{s_ders}: {s_konu}"
                            target_col = {"Pazartesi": "pazartesi", "Salı": "sali", "Çarşamba": "carsamba", "Perşembe": "persembe", "Cuma": "cuma", "Cumartesi": "cumartesi", "Pazar": "pazar"}[g_adi]
                            cursor.execute(f"INSERT INTO excel_program_matris (ad_soyad, saat_araligi, {target_col}) VALUES (?, ?, ?) ON CONFLICT(ad_soyad, saat_araligi) DO UPDATE SET {target_col} = ?", (secilen_ogr, s_saat, icerik, icerik))
                            conn.commit()
