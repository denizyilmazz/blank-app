    ...
    with main_tab2:
        ...
        if not st.session_state["aktif_koc"]:
            ...
            if ogrenci_rows:
                ...
                ogr_secilen_res = cursor.fetchone()
                s_turu_val = ogr_secilen_res[0] if ogr_secilen_res else "TYT (Sadece TYT Çalışması)"

                if "TYT (Sadece" in s_turu_val: KOC_K_DERSLER = list(TYT_KONULAR.keys())
