...
def haftalik_program_toplu_pdf_bytes(df_full, ogrenci_adi):
    ...
    gunler = [("Pazartesi", "pazartesi"), ("Salı", "sali"), ("Çarşamba", "carsamba"), ("Perşembe", "persembe"), ("Cuma", "cuma"), ("Cumartesi", "cumartesi"), ("Pazar", "pazar")]
    gun_htmls = ""
    ...
    <!DOCTYPE html>
    <html lang="tr">
    <head><meta charset="UTF-8"><title>{ogrenci_adi} - Toplu Haftalık Ders Programı</title></head>
    <body style="font-family:'Plus Jakarta Sans',sans-serif; padding:30px; color:#0f172a;">
        <h2 style="text-align:center; color:#0284c7;">🎓 YKS KOÇLUK — {ogrenci_adi.upper()} TOPLU HAFTALIK DERS PROGRAMI</h2>
        <p style="text-align:center; color:#64748b; font-size:12px;">Deniz Yılmaz Gelişim Sistemleri | {datetime.date.today().strftime('%d.%m.%Y')}</p>
        {gun_htmls}
    </body>
    </html>
    """
...
HAM_DERS_KONULARI = {
    "🌅 Günlük Rutinler": ["Paragraf 25 Soru + Problem 20 Soru"],
    "☕ Mola & Dinlenme Aktivitesi": ["Kısa Dinlenme & Çay/Kahve Molası", "Zihin Dinlendirme Mola"],
    "🍽️ Yemek Molaları": ["Öğle Yemeği Molası", "Akşam Yemeği Molası"],
    ...
    "🧬 TYT Biyoloji": ["Canlıların Ortak Özellikleri ve Temel Bileşenler", "Hücre ve Organelleri", "Madde Geçişleri", "Hücre Bölünmeleri (Mitoz ve Mayoz)", "Kalıtım ve Evrim", "Ekoloji"],
    "📜 TYT Tarih": ["Tarih Bilimi", "İlk Çağ Medeniyetleri", "İslamiyet Tarihi ve Uygarlığı", "Türklerin İslamiyet'i Kabulü ve İlk Türk Devletleri", "Osmanlı Devleti Kuruluş ve Yükselme", "Osmanlı Kültür ve Medeniyeti", "Milli Mücadele Dönemi", "Atatürk İnkılap ve İlkeleri"],
    "🌍 TYT Coğrafya": ["Doğa ve İnsan & Harita Bilgisi", "Dünyanın Şekli ve Hareketleri", "İklim Bilgisi", "İç ve Dış Kuvvetler", "Nüfus ve Yerleşme", "Ulaşım Yolları ve Göç", "Afetler"],
    "🧠 TYT Felsefe": ["Felsefeyi Tanıma", "Bilgi Felsefesi (Epistemoloji)", "Varlık Felsefesi (Ontoloji)", "Ahlak Felsefesi", "Din, Siyaset ve Sanat Felsefesi"],
    "🕌 TYT Din Kültürü": ["İnanç", "İbadet", "Ahlak ve Değerler", "Hz. Muhammed'in Hayatı, Örnekliği ve Hz. Muhammed", "Vahiy ve Akıl"],
    "📐 AYT Matematik": ["İkinci Dereceden Denklemler ve Karmaşık Sayılar", "Parabol", "İkinci Dereceden Eşitsizlikler", "Logaritma", "Diziler", "Trigonometri", "Limit ve Süreklilik", "Türev", "İntegral ve Alan"],
    "📏 AYT Geometri": ["Çemberin Analitik İncelenmesi", "Dönüşüm Geometrisi", "Trigonometri Geometri Uygulamaları"],
    "⚡ AYT Fizik": ["Vektörler ve Bağıl Hareket", "Dinamik (Newton'ın Hareket Yasaları)", "Bir Boyutta ve İki Boyutta Sabit İvmeli Hareket (Atışlar)", "İş, Güç ve Enerji (AYT)", "İtme ve Çizgisel Momentum", "Tork, Denge ve Kütle Merkezi", "Basit Makineler", "Elektriksel Kuvvet, Elektrik Alan ve Potansiyel", "Sığalar (Kondansatörler)", "Manyetik Alan ve Manyetik Kuvvet", "İndüksiyon, Özindüksiyon ve Alternatif Akım", "Çembersel Hareket ve Kepler Yasaları", "Basit Harmonik Hareket", "Dalga Mekaniği (Girişim, Kırınım) ve Elektromanyetik Dalgalar", "Atom Fiziği, Radyoaktivite ve Modern Fizik"],
...
OSYM_SORU_DAGILIMLARI = {k: "ÖSYM Ort. 1-2 Soru" for v in HAM_DERS_KONULARI.values() for k in v}
OSYM_SORU_DAGILIMLARI.update({
...
BOLUM_KATEGORILERI = {
    "SAY (Sayısal)": ["Matematik", "Tıp Fakültesi", "Bilgisayar Mühendisliği", "Elektrik-Elektronik Mühendisliği", "Endüstri Mühendisliği", "Makine Mühendisliği", "Yazılım Mühendisliği", "Mimarlık", "Diş Hekimliği Fakültesi", "Eczacılık Fakültesi", "Hemşirelik"],
    "EA (Eşit Ağırlık)": ["Hukuk Fakültesi", "Psikoloji", "İşletme", "İktisat", "Yönetim Bilişim Sistemleri", "Rehberlik ve Psikolojik Danışmanlık (PDR)", "Sınıf Öğretmenliği"],
    "SÖZ (Sözel)": ["Türk Dili ve Edebiyatı", "Tarih", "Gastronomi ve Mutfak Sanatları", "İlahiyat Fakültesi", "Türkçe Öğretmenliği"],
    "DİL (Yabancı Dil)": ["İngilizce Öğretmenliği", "Tercümanlık ve Çeviribilim", "İngiliz Dili ve Edebiyatı"]
}

def akilli_taslak_olustur_matris(alan="SAY (Sayısal)"):
    saatler = ["08:00 - 09:00", "09:15 - 10:15", "10:30 - 11:30", "13:30 - 14:30", "14:45 - 15:45", "19:00 - 20:00", "20:15 - 21:15"]
    ...
    for s in saatler:
        ...
        for g_idx, g in enumerate(gunler):
            if "Mola" in s or g_idx == 6 and "20:" in s:
                row[g] = "☕ Dinlenme / Serbest Zaman"
            elif g_idx == 5 and g_idx < 6:
                row[g] = "📊 Branş Denemesi Analizi & Tekrar"
            else:
                chosen = ders_havuzu[(g_idx + hash(s)) % len(ders_havuzu)]
                row[g] = f"🌅 Günlük Rutinler\n↳ Paragraf 25 Soru" if s.startswith("08:") else chosen
        rows.append(row)
    ...
    with main_tab1:
        ...
        if not aktif_ogr:
                        ...
                        if login_ad and login_sifre:
                            ...
                            if usr and verify_hash(login_sifre, usr[0]):
                                if usr[1] == 1:
                                    ...
                                    if beni_hatirla_ogr: st.query_params["hatirla_ogr"] = login_ad
                                    else:
                                        if "hatirla_ogr" in st.query_params: del st.query_params["hatirla_ogr"]
                                    st.rerun()
                                else: st.warning("⏳ Hesabınız koçunuz tarafından henüz onaylanmamıştır.")
                            else: st.error("❌ Hatalı ad veya şifre!")
            ...
            tab_hedef, tab_program, tab_ilerleme, tab_gunluk, tab_deneme, tab_konular, tab_uyku = st.tabs([
                "🎯 YÖK ATLAS & ÖSYM", "📅 DERS PROGRAMI", "✅ İLERLEME TAKİBİ", "📝 GÜNLÜK ÇALIŞMA", "📊 DENEME YÜKLEME", "🗺️ KONU HAKİMİYETİ", "😴 UYKU TAKİBİ"
            ])
            ...
            with tab_hedef:
                ...
                with col_h_u2: secilen_kategori = st.selectbox("Puan Türü / Kategori:", list(BOLUM_KATEGORILERI.keys()))
                ...
                conn_det.close()
                t_net, t_sira, tyt_gerekli, ayt_gerekli = ozel_kayit if ozel_kayit else (85.0, "50.000", 80.0, 52.0)
                st.markdown(f"""
                <div class="yok-net-box">
                    ...
                    <div style="display: flex; gap: 20px; flex-wrap: wrap; margin-top: 10px;">
                        <div style="background: var(--container-bg); padding: 10px 15px; border-radius: 12px; border: 1px solid var(--border-color); flex: 1; min-width: 140px;"><span style="font-size: 11px;">YÖK ATLAS NET</span><br><b>{t_net} Net</b></div>
                        <div style="background: var(--container-bg); padding: 10px 15px; border-radius: 12px; border: 1px solid var(--border-color); flex: 1; min-width: 140px;"><span style="font-size: 11px;">SIRALAMA</span><br><b>İlk {t_sira}</b></div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                if st.button("🚀 Bu Hedefi Profilime Kaydet", type="primary", use_container_width=True):
                    conn_up = get_db_connection()
                    cur_up = conn_up.cursor()
                    cur_up.execute("UPDATE ogrenciler SET hedef_uni = %s, hedef_bolum = %s, hedef_net = %s WHERE ad_soyad = %s", (secilen_hedef_uni, f"{secilen_hedef_bolum} ({secilen_kategori})", float(t_net), aktif_ogr))
                    conn_up.commit(); conn_up.close()
                    st.success("🎉 Hedef güncellendi!"); st.rerun()

            with tab_program:
                st.markdown(f'<div class="program-header-box"><h2 style="margin:0; font-size:22px; color:white !important;">📅 {aktif_ogr.upper()} — DERS PROGRAMI</h2></div>', unsafe_allow_html=True)
                bugun_tarih = datetime.date.today()
                secilen_hafta_str = str(bugun_tarih - datetime.timedelta(days=bugun_tarih.weekday()))
                ogr_prog_alt_secim = st.radio("Görünüm:", ["☀️ Bugün", "📅 Tüm Hafta"], horizontal=True)
                gun_indexleri = {0: "pazartesi", 1: "sali", 2: "carsamba", 3: "persembe", 4: "cuma", 5: "cumartesi", 6: "pazar"}
                bugun_kolun = gun_indexleri[bugun_tarih.weekday()]
                ...
                if not df_p_full.empty:
                    if ogr_prog_alt_secim == "☀️ Bugün":
                        df_bugun = df_p_full[["saat_araligi", bugun_kolun]].rename(columns={"saat_araligi": "Saat Aralığı", bugun_kolun: "Ders / Aktivite"})
                        df_bugun = df_bugun[df_bugun["Ders / Aktivite"].notna() & (df_bugun["Ders / Aktivite"].astype(str).str.strip() != "")]
                        st.dataframe(df_bugun, use_container_width=True, hide_index=True)
                    else:
                        st.dataframe(df_p_full, use_container_width=True, hide_index=True)
                        st.download_button("📥 Toplu PDF İndir", data=haftalik_program_toplu_pdf_bytes(df_p_full, aktif_ogr), file_name=f"{aktif_ogr}_program.html", mime="text/html", use_container_width=True)
                else: st.info("ℹ️ Bu hafta için henüz program girilmedi.")

            with tab_ilerleme:
                st.markdown(f"### ✅ Konu İlerleme & Net Takibi — {aktif_ogr}")
                secilen_takip_ders = st.selectbox("Ders Seçin:", list(HAM_DERS_KONULARI.keys()), key="takip_ders_secim")
                conn_t = get_db_connection()
                cur_t = conn_t.cursor()
                takip_verileri = []
                for konu in HAM_DERS_KONULARI[secilen_takip_ders]:
                    ...
                    d_val, y_val, b_val = (res[2], res[3], res[4]) if (res and len(res)>4 and res[2] is not None) else (0,0,0)
                    s_val = int(res[1]) if (res and res[1] is not None) else (d_val+y_val+b_val)
                    takip_verileri.append({"Konu Adı": konu, "Tamamlandı ✅": t_val, "Doğru": d_val, "Yanlış": y_val, "Boş": b_val, "Toplam Çözülen": s_val})
                conn_t.close()
                df_takip = pd.DataFrame(takip_verileri)
                with st.form(f"ilerleme_form_{secilen_takip_ders}"):
                    edited_takip = st.data_editor(df_takip, use_container_width=True, hide_index=True, disabled=["Konu Adı", "Toplam Çözülen"])
                    if st.form_submit_button("💾 Kaydet", type="primary", use_container_width=True):
                        conn_sv = get_db_connection()
                        cur_sv = conn_sv.cursor()
                        for _, row in edited_takip.iterrows():
                            d_v, y_v, b_v = int(row["Doğru"]), int(row["Yanlış"]), int(row["Boş"])
                            cur_sv.execute("INSERT INTO konu_ilerleme (ad_soyad, ders, konu_adi, tamamlandi, soru_miktari, dogru, yanlis, bos) VALUES (%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT(ad_soyad,ders,konu_adi) DO UPDATE SET tamamlandi=EXCLUDED.tamamlandi, soru_miktari=EXCLUDED.soru_miktari, dogru=EXCLUDED.dogru, yanlis=EXCLUDED.yanlis, bos=EXCLUDED.bos",
                                           (aktif_ogr, secilen_takip_ders, row["Konu Adı"], 1 if row["Tamamlandı ✅"] else 0, d_v+y_v+b_v, d_v, y_v, b_v))
            ...
            with tab_gunluk:
                ...
                secilen_ders = st.selectbox("Ders Seçin:", list(EVRENSEL_DERS_KONULARI.keys()), key="g_ders")
                secilen_konu = st.selectbox("Konu Seçin:", EVRENSEL_DERS_KONULARI.get(secilen_ders, ["Genel"]), key="g_konu")
                cn1, cn2, cn3 = st.columns(3)
                with cn1: g_dogru = st.number_input("Doğru", 0, 500, 15)
                with cn2: g_yanlis = st.number_input("Yanlış", 0, 500, 3)
                with cn3: g_bos = st.number_input("Boş", 0, 500, 2)
                tot_soru = g_dogru + g_yanlis + g_bos
                st.info(f"📊 Toplam: {tot_soru} Soru")
                cs1, cs2 = st.columns(2)
                with cs1: g_k_sure = st.number_input("Konu Süre (dk)", 0, 1440, 45)
                with cs2: g_c_sure = st.number_input("Çözüm Süre (dk)", 0, 1440, 45)
                yapilamayan_fotolar = st.file_uploader("Boş / Yapılamayan Görseller:", type=["png", "jpg", "jpeg"], accept_multiple_files=True)
                if st.button("🚀 Çalışmayı Kaydet", type="primary", use_container_width=True):
                    conn_g = get_db_connection()
                    cur_g = conn_g.cursor()
                    cur_g.execute("INSERT INTO gunluk_calisma (ad_soyad, tarih, ders, konu, soru_sayisi, konu_anlatim_sure, soru_cozum_sure, dogru, yanlis, bos) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                  (aktif_ogr, str(s_tarih), secilen_ders, secilen_konu, tot_soru, g_k_sure, g_c_sure, g_dogru, g_yanlis, g_bos))
                    if yapilamayan_fotolar:
                        for p in yapilamayan_fotolar:
                            p_path = os.path.join(COZULEMEYEN_DIR, f"{datetime.date.today()}_{aktif_ogr}_{random.randint(100,999)}_{p.name}")
                            with open(p_path, "wb") as f: f.write(p.getbuffer())
                            cur_g.execute("INSERT INTO yapilamayan_sorular (ad_soyad, tarih, ders, konu, dosya_yolu, dosya_adi) VALUES (%s,%s,%s,%s,%s,%s)", (aktif_ogr, str(s_tarih), secilen_ders, secilen_konu, p_path, p.name))
                    conn_g.commit(); conn_g.close()
                    st.success("🎉 Kaydedildi!"); st.rerun()

                st.markdown("#### 🗑️ Hatalı Kayıt Sil")
                conn_del_q = get_db_connection()
                df_son = pd.read_sql_query('SELECT id, tarih, ders, konu, soru_sayisi FROM gunluk_calisma WHERE ad_soyad = %s ORDER BY id DESC LIMIT 10', conn_del_q.conn, params=(aktif_ogr,))
                conn_del_q.close()
                if not df_son.empty:
                    opts = [f"ID:{r['id']} | {r['tarih']} | {r['ders']} ({r['soru_sayisi']} s)" for _, r in df_son.iterrows()]
                    del_choice = st.selectbox("Silinecek Kayıt:", opts)
                    if st.button("Sil", type="secondary"):
                        did = int(del_choice.split("|")[0].replace("ID:", "").strip())
                        conn_del = get_db_connection()
                        conn_del.cursor().execute("DELETE FROM gunluk_calisma WHERE id=%s", (did,))
            ...
            with tab_konular:
                ...
                for d_adi, k_list in HAM_DERS_KONULARI.items():
                    st.markdown(f"**{d_adi}**")
                    for kn in k_list[:2]: st.select_slider(kn, options=[1,2,3,4,5], value=3, key=f"kp_{aktif_ogr}_{kn}")

            with tab_uyku:
                st.markdown(f"### 😴 Uyku Takibi — {aktif_ogr}")
                with st.form("uyku_f"):
                    ut = st.date_input("Tarih:", datetime.date.today())
                    us = st.number_input("Süre (Saat):", 0.0, 16.0, 7.5)
                    uk = st.selectbox("Kalite:", ["Dinç / Süper ⚡", "Normal 👍", "Yorgun 🥱", "Kötü 👎"])
                    if st.form_submit_button("Kaydet", type="primary"):
                        conn_u = get_db_connection()
                        conn_u.cursor().execute("INSERT INTO uyku_takibi (ad_soyad, tarih, uyku_suresi, uyku_kalitesi) VALUES (%s,%s,%s,%s) ON CONFLICT(ad_soyad,tarih) DO UPDATE SET uyku_suresi=EXCLUDED.uyku_suresi, uyku_kalitesi=EXCLUDED.uyku_kalitesi", (aktif_ogr, str(ut), float(us), uk))
                        conn_u.commit(); conn_u.close()
                        st.success("Uyku kaydedildi!"); st.rerun()
                conn_uh = get_db_connection()
                df_u = pd.read_sql_query('SELECT tarih AS "Tarih", uyku_suresi AS "Saat", uyku_kalitesi AS "Kalite" FROM uyku_takibi WHERE ad_soyad = %s ORDER BY tarih DESC LIMIT 7', conn_uh.conn, params=(aktif_ogr,))
                conn_uh.close()
        ...
        if not st.session_state["aktif_koc"]:
            with st.form("koc_g"):
                ...
                if st.form_submit_button("Giriş Yap", type="primary"):
                    ...
                    conn_kg.cursor().execute("SELECT sifre FROM koclar WHERE kullanici_adi = %s", (k_ad,))
                    conn_kg.close()
                    st.session_state["aktif_koc"] = k_ad; st.rerun()
        else:
            if st.button("🚪 Çıkış Yap"): st.session_state["aktif_koc"] = None; st.rerun()
            conn_ogrs = get_db_connection()
            ogrs = [row[0] for row in conn_ogrs.cursor().execute('SELECT ad_soyad FROM ogrenciler WHERE onaylandi = 1') or conn_ogrs.cursor().fetchall()]
            conn_ogrs.close()
            if ogrs:
                secilen_ogr = st.selectbox("Yönetilecek Öğrenci:", ogrs)
                st.markdown(f"### 🗓️ {secilen_ogr} — Hibrit Akıllı Program Düzenleyici")
                ...
                if mod_secim == "⚡ AI Akıllı Taslak Üretici":
                    st.info("💡 Öğrencinin alanına göre 35 saatlik dengeli haftalık programı 1 saniyede taslak olarak doldurur.")
                    ogr_alan_info = "SAY (Sayısal)"
                    conn_oalan = get_db_connection()
                    cur_oa = conn_oalan.cursor()
                    cur_oa.execute("SELECT alan FROM ogrenciler WHERE ad_soyad = %s", (secilen_ogr,))
                    res_oa = cur_oa.fetchone()
                    if res_oa and res_oa[0]: ogr_alan_info = res_oa[0]
                    conn_oalan.close()
                    
                    if st.button(f"⚡ {ogr_alan_info} İçin Akıllı Taslak Üret ve Yaz", type="primary"):
                        df_draft = akilli_taslak_olustur_matris(ogr_alan_info)
                        conn_tr = get_db_connection()
                        cur_tr = conn_tr.cursor()
                        cur_tr.execute("DELETE FROM excel_program_matris WHERE ad_soyad = %s AND hafta_baslangici = %s", (secilen_ogr, koc_hafta_str))
                        for _, dr in df_draft.iterrows():
                            cur_tr.execute("INSERT INTO excel_program_matris (ad_soyad, hafta_baslangici, saat_araligi, pazartesi, sali, carsamba, persembe, cuma, cumartesi, pazar) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                           (secilen_ogr, koc_hafta_str, dr["Saat Aralığı"], dr["pazartesi"], dr["sali"], dr["carsamba"], dr["persembe"], dr["cuma"], dr["cumartesi"], dr["pazar"]))
                        conn_tr.commit(); conn_tr.close()
                        st.success("🎉 Akıllı taslak uygulandı!"); st.rerun()

                elif mod_secim == "🔄 Hızlı Blok Swap / Taşıma":
                    st.caption("Bir günden diğerine ders kaydırma / yer değiştirme asistanı:")
                    c_sw1, c_sw2, c_sw3 = st.columns(3)
                    with c_sw1: source_gun = st.selectbox("Kaynak Gün:", ["pazartesi", "sali", "carsamba", "persembe", "cuma", "cumartesi", "pazar"])
                    with c_sw2: target_gun = st.selectbox("Hedef Gün:", ["pazartesi", "sali", "carsamba", "persembe", "cuma", "cumartesi", "pazar"])
                    with c_sw3: hedef_saat = st.text_input("Saat Aralığı (Örn: 09:15 - 10:15):", "09:15 - 10:15")
                    ders_tasi_not = st.text_input("Taşınacak Ders İçeriği:")
                    if st.button("🔄 Bloğu Taşı / İşle"):
                        conn_sw = get_db_connection()
                        conn_sw.cursor().execute(f"UPDATE excel_program_matris SET {target_gun} = %s WHERE ad_soyad = %s AND hafta_baslangici = %s AND saat_araligi = %s", (ders_tasi_not, secilen_ogr, koc_hafta_str, hedef_saat))
                        conn_sw.commit(); conn_sw.close()
                        st.success("Blok güncellendi!"); st.rerun()

                conn_m = get_db_connection()
                df_matris = pd.read_sql_query('SELECT saat_araligi AS "Saat Aralığı", pazartesi AS "pazartesi", sali AS "sali", carsamba AS "carsamba", persembe AS "persembe", cuma AS "cuma", cumartesi AS "cumartesi", pazar AS "pazar" FROM excel_program_matris WHERE ad_soyad = %s AND hafta_baslangici = %s ORDER BY saat_araligi ASC', conn_m.conn, params=(secilen_ogr, koc_hafta_str))
                conn_m.close()
                if df_matris.empty:
                    df_matris = pd.DataFrame([{"Saat Aralığı": "08:00 - 09:00", "pazartesi": "", "sali": "", "carsamba": "", "persembe": "", "cuma": "", "cumartesi": "", "pazar": ""}])
                
                edited_matris = st.data_editor(df_matris, num_rows="dynamic", use_container_width=True, height=400, key=f"mat_{secilen_ogr}_{koc_hafta_str}")
                if st.button("💾 Matris Değişikliklerini Kaydet", type="primary", use_container_width=True):
                    ...
                    for _, row in edited_matris.iterrows():
                        ...
                        if s_ar and s_ar != "nan":
                            cur_sv2.execute("INSERT INTO excel_program_matris (ad_soyad, hafta_baslangici, saat_araligi, pazartesi, sali, carsamba, persembe, cuma, cumartesi, pazar) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                            (secilen_ogr, koc_hafta_str, s_ar, str(row.get("pazartesi","")), str(row.get("sali","")), str(row.get("carsamba","")), str(row.get("persembe","")), str(row.get("cuma","")), str(row.get("cumartesi","")), str(row.get("pazar",""))))
